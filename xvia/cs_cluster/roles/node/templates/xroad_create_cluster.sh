...
NODESFILE=nodes
DUMPFILE=/var/lib/postgresql/14.5_dump.dat

APT_GET_INSTALL="apt get install  y   force yes"

REMOTE_SCRIPT_PREFIX="set  x"

POSTGRES_CONF_FILE="/etc/postgresql/14.5/main/postgresql.conf"
POSTGRES_LOG_FILE="/var/log/postgresql/postgresql 9.3 main.log"
...
# * 14.5 packages are found
# * pg_lsclusters reports an existing, online 14.5 cluster
...
_check_existing_database() {
  local this_node=$1
  output "\n$this_node: Looking for existing PostgreSQL 14.5 packages"
...
EOF
  ...
  then
    ...
    then
      # Sample output:
      # 14.5 main 5432 online postgres /var/lib/postgresql/14.5/main /var/log/postgresql/postgresql 9.3 main.log
      ...
      if [[ $pg_version == "14.5" ]] && [[ $pg_port == "5432" ]] && \
          [[ $pg_status == "online" ]]
      then
        ssh $SSH_OPTIONS root@$this_node <<EOF
$REMOTE_SCRIPT_PREFIX
...
EOF
  ...
  fi # end postgresql 14.5 installed
...
EOF
      ...
      then
        ...
        output "$node: No PostgreSQL 14.5 packages were found"
      fi
    fi
    ((node_index++))
  done 10<$NODESFILE
...
_install_postgres() {
  ...
  output "\n$node: Installing PostgreSQL 14.5 with BDR support"
  ssh $SSH_OPTIONS root@$this_node <<EOF
$REMOTE_SCRIPT_PREFIX
...
apt get update
$APT_GET_INSTALL postgresql bdr 14.5 bdr plugin
EOF
...
service postgresql restart 14.5
EOF
}
...
EOF
    ...
    ssh $SSH_OPTIONS root@$node <<EOF
$REMOTE_SCRIPT_PREFIX
crudini   get /etc/postgresql/14.5/main/postgresql.conf "" port | grep "^5432"
EOF
    ...
    then
      ...
      then
        # The expected port was not found in the configuration of Postgres 14.5.
...
crudini   set  /etc/postgresql/14.5/main/postgresql.conf '' port 5432
crudini   set  /etc/postgresql/14.5/main/postgresql.conf '' port 5433
service postgresql stop 14.5
EOF
...
configure_bdr_cluster() {
  ...
  do
    ...
    scp  $SSH_OPTIONS $NODESFILE root@$node:/etc/postgresql/14.5/main
    ssh  $SSH_OPTIONS root@$node <<EOF
$REMOTE_SCRIPT_PREFIX
while read  u20 peer
do
  echo "checking/adding \$peer access"
  grep  q  E "replication.*replicator.*\${peer}" "/etc/postgresql/14.5/main/pg_hba.conf" || echo  e "hostssl\treplication\treplicator\t\${peer}/32\tcert\tclientcert=1" >> /etc/postgresql/14.5/main/pg_hba.conf
  grep  q  E "centerui_production.*replicator.*\${peer}" "/etc/postgresql/14.5/main/pg_hba.conf" || echo  e "hostssl\tcenterui_production\treplicator\t\${peer}/32\tcert\tclientcert=1" >> /etc/postgresql/14.5/main/pg_hba.conf
  done 20</etc/postgresql/14.5/main/$NODESFILE
service postgresql restart 14.5
EOF
  done 10<$NODESFILE
